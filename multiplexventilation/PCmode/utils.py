...
class PCModel:
    ...
    def simulate(self):
        ...
        P1 = lambda t: A*(t-t1[-1]) + PIP
        P2 = lambda t: PIP
        P3 = lambda t: B*(t-t3[-1]) + PEEP
        P4 = lambda t: PEEP

        # Defining the function for first order ODE:
        dv_dt1 = lambda v,t: ( P1(t) - PEEP - E*v)/R
        dv_dt2 = lambda v,t: ( P2(t) - PEEP - E*v)/R
        dv_dt3 = lambda v,t: ( P3(t) - PEEP - E*v)/R
        dv_dt4 = lambda v,t: ( P4(t) - PEEP - E*v)/R
        ...
        P = py.concatenate([P1(t1),P2(t2)*py.ones(len(t2)),P3(t3),P4(t4)*py.ones(len(t4))])
        ...
        VT = round(max(V)*1000/Weight,2) # Tidal volume for the current patient
        ...
        figP1.update_xaxes(title_text="<b>Time(s)<b>", title_font=dict(size=18), row=3, col=1) # Similar for all graph

        # Update yaxis properties
        figP1.update_yaxes(title_text="<b>Pressure (cmH2O)<b>", title_font=dict(size=15), row=1, col=1) 
        figP1.update_yaxes(title_text="<b>Flow (L/s)<b>", title_font=dict(size=15),  row=2, col=1) 
        figP1.update_yaxes(title_text="<b>Volume (L)<b>", title_font=dict(size=15),  row=3, col=1) 
...
class PCPairing:
    ...
    def simulate(self):
        ...
        for k in range(len(Weight)):   
            ...
            # Rearranging 
            if V_temp:
                E_temp, R_temp, V_temp = zip(*sorted(zip(E_temp, R_temp, V_temp)))
        ...
        figPair.update_xaxes(title_text="<b>Elastance (cmH2O/L)<b>", title_font=dict(size=18))
        figPair.update_yaxes(title_text="<b>Resistance (cmH2O.s/L)<b>", title_font=dict(size=18))
...
class PCSetting:
    ...
    def simulate(self):
        ...
        # Defining equation of pressure for each time
        Pj1 = lambda t: A*(t-t1[-1]) + PIP
        Pj2 = lambda t: PIP
        Pj3 = lambda t: B*(t-t3[-1]) + PEEP
        Pj4 = lambda t: PEEP

        # Solving for the corresponding Flow and Volume analytically
        #----------------------------------Patient 1------------------------------------------------------------
        # Defining function handle for Volume
        C11 = ((A/E1)*(-t1[0]+t1[-1]+timeconst1)- dP/E1)*py.exp(t1[0]/timeconst1)
        V1P1 = lambda t: dP/E1 + (A/E1)*(t-t1[-1]) - A*timeconst1/E1 + C11*py.exp(-t/timeconst1)
        C21 = (V1P1(t1[-1]) - dP*timeconst1/R1) * py.exp(t1[-1]/timeconst1)
        V2P1 = lambda t: dP*timeconst1/R1 + C21*py.exp(-t/timeconst1)
        C31 = (R1*V2P1(t2[-1]) - B*timeconst1*(t2[-1]-t3[-1]-timeconst1))*py.exp(t2[-1]/timeconst1)
        V3P1 = lambda t: (B*timeconst1*(t-t3[-1]-timeconst1) + C31*py.exp(-t/timeconst1))/R1
        C41 = V3P1(t3[-1])*py.exp(t3[-1]/timeconst1)
        V4P1 = lambda t: C41*py.exp(-t/timeconst1)


        # Defining function handle for Flow
        Q1P1 = lambda t: A/E1 - C11*py.exp(-t/timeconst1)/timeconst1
        Q2P1 = lambda t: - C21*py.exp(-t/timeconst1)/timeconst1
        Q3P1 = lambda t: (B*timeconst1 - C31*py.exp(-t/timeconst1)/timeconst1)/R1
        Q4P1 = lambda t: -C41*py.exp(-t/timeconst1)/timeconst1
        ...
        #--------------------------------Patient 2------------------------------------------------------------
        # Defining function handle for volume
        C12 = ((A/E2)*(-t1[0]+t1[-1]+timeconst2)- dP/E2)*py.exp(t1[0]/timeconst2)
        V1P2 = lambda t: dP/E2 + (A/E2)*(t-t1[-1]) - A*timeconst2/E2 + C12*py.exp(-t/timeconst2)
        C22 = (V1P2(t1[-1]) - dP*timeconst2/R2) * py.exp(t1[-1]/timeconst2)
        V2P2 = lambda t: dP*timeconst2/R2 + C22*py.exp(-t/timeconst2)
        C32 = (R2*V2P2(t2[-1]) - B*timeconst2*(t2[-1]-t3[-1]-timeconst2))*py.exp(t2[-1]/timeconst2)
        V3P2 = lambda t: (B*timeconst2*(t-t3[-1]-timeconst2) + C32*py.exp(-t/timeconst2))/R2
        C42 = V3P2(t3[-1])*py.exp(t3[-1]/timeconst2)
        V4P2 = lambda t: C42*py.exp(-t/timeconst2)

        # Defining function handle for Flow
        Q1P2 = lambda t: A/E2 - C12*py.exp(-t/timeconst2)/timeconst2
        Q2P2 = lambda t: - C22*py.exp(-t/timeconst2)/timeconst2
        Q3P2 = lambda t: (B*timeconst2 - C32*py.exp(-t/timeconst2)/timeconst2)/R2
        Q4P2 = lambda t: -C42*py.exp(-t/timeconst2)/timeconst2
        ...
        # Patient 1
        dQ1_dtP1 = lambda t: (1/(timeconst1)**2)*C11*py.exp(-t/timeconst1)
        dQ2_dtP1 = lambda t: (1/(timeconst1)**2)*C21*py.exp(-t/timeconst1)
        dQ3_dtP1 = lambda t: (1/(R1*(timeconst1)**2))*C31*py.exp(-t/timeconst1)
        dQ4_dtP1 = lambda t: (1/(timeconst1)**2)*C41*py.exp(-t/timeconst1)

        # Patient 2
        dQ1_dtP2 = lambda t: (1/(timeconst2)**2)*C12*py.exp(-t/timeconst2)
        dQ2_dtP2 = lambda t: (1/(timeconst2)**2)*C22*py.exp(-t/timeconst2)
        dQ3_dtP2 = lambda t: (1/(R2*(timeconst2)**2))*C32*py.exp(-t/timeconst2)
        dQ4_dtP2 = lambda t: (1/(timeconst2)**2)*C42*py.exp(-t/timeconst2)


        # Pressure (minimum setting on ventilator)
        P1 = lambda t: E1*V1P1(t) + (R1+Rc)*Q1P1(t) + Rc*Q1P2(t) + PEEP
        P2 = lambda t: E1*V2P1(t) + (R1+Rc)*Q2P1(t) + Rc*Q2P2(t) + PEEP
        P3 = lambda t: E1*V3P1(t) + (R1+Rc)*Q3P1(t) + Rc*Q3P2(t) + PEEP
        P4 = lambda t: E1*V4P1(t) + (R1+Rc)*Q4P1(t) + Rc*Q4P2(t) + PEEP

        # dpdt 
        dP1_dt = lambda t: E1*Q1P1(t) + (R1+Rc)*dQ1_dtP1(t) + Rc*dQ1_dtP2(t)
        dP2_dt = lambda t: E1*Q2P1(t) + (R1+Rc)*dQ2_dtP1(t) + Rc*dQ2_dtP2(t)
        dP3_dt = lambda t: E1*Q3P1(t) + (R1+Rc)*dQ3_dtP1(t) + Rc*dQ3_dtP2(t)
        dP4_dt = lambda t: E1*Q4P1(t) + (R1+Rc)*dQ4_dtP1(t) + Rc*dQ4_dtP2(t)

        # Defining the function for second order ODE:
        f1 = lambda x,t: [x[1],(R*dP1_dt(t) + E*(P1(t)-PEEP) - ((R2 + Rc)*E1 + (R1 + Rc)*E2)*x[1] - E1*E2*x[0])/(R1*R2 + Rc*R)]
        f2 = lambda x,t: [x[1],(R*dP2_dt(t) + E*(P2(t)-PEEP) - ((R2 + Rc)*E1 + (R1 + Rc)*E2)*x[1] - E1*E2*x[0])/(R1*R2 + Rc*R)]
        f3 = lambda x,t: [x[1],(R*dP3_dt(t) + E*(P3(t)-PEEP) - ((R2 + Rc)*E1 + (R1 + Rc)*E2)*x[1] - E1*E2*x[0])/(R1*R2 + Rc*R)]
        f4 = lambda x,t: [x[1],(R*dP4_dt(t) + E*(P4(t)-PEEP) - ((R2 + Rc)*E1 + (R1 + Rc)*E2)*x[1] - E1*E2*x[0])/(R1*R2 + Rc*R)]
        ...
        Pmax = round(mean(P2(t2)),0); # Minimum pressure to obtain the desired result (Obtained from the ramp section)
        ...
        Pv1 = lambda t: X*(t-t1[-1]) + Pmax
        Pv2 = lambda t: Pmax
        Pv3 = lambda t: Y*(t-t3[-1]) + PEEP
        Pv4 = lambda t: PEEP
        ...
        # Defining the function for second order ODE:
        g1 = lambda x,t: [x[1], (R*dPv1_dt(t) + E*(Pv1(t)-PEEP) - ((R2 + Rc)*E1 + (R1 + Rc)*E2)*x[1] - E1*E2*x[0])/(R1*R2 + Rc*R)]
        g2 = lambda x,t: [x[1], (R*dPv2_dt(t) + E*(Pv2(t)-PEEP) - ((R2 + Rc)*E1 + (R1 + Rc)*E2)*x[1] - E1*E2*x[0])/(R1*R2 + Rc*R)]
        g3 = lambda x,t: [x[1], (R*dPv3_dt(t) + E*(Pv3(t)-PEEP) - ((R2 + Rc)*E1 + (R1 + Rc)*E2)*x[1] - E1*E2*x[0])/(R1*R2 + Rc*R)]
        g4 = lambda x,t: [x[1], (R*dPv4_dt(t) + E*(Pv4(t)-PEEP) - ((R2 + Rc)*E1 + (R1 + Rc)*E2)*x[1] - E1*E2*x[0])/(R1*R2 + Rc*R)]
        ...
        def picardI(fequation, prange, xrange, yo, precision):
            error = 1
            ytemp = yo + cumtrapz(fequation(prange, py.ones(len(xrange))*yo), xrange, initial=0)
            i = 0
        ...
        #---------------------------------------------Patient 1----------------------------------------------------
        dv_dt = lambda P,v: (P - PEEP - E1*v)/R1
        Vo = 0
        ...
        #--------------------------------Patient 2----------------------------------------------------
        dv_dt = lambda P,v: (P - PEEP - E2*v)/R2
        ...
        # Storing the values
        P = py.concatenate([Pv1(t1), py.ones(len(t2))*Pv2(t2), Pv3(t3), py.ones(len(t4))*Pv4(t4)])
        ...
        figPtab = make_subplots(rows=2, cols=1, 
                                    ...
                                    subplot_titles=("Recommended Ventilator Setting","Patients"),
                                    specs=[[{"type": "table"}],
                                        [{"type": "table"}],
                                        ])
        ...
        figPset.add_trace(go.Scatter(x=t,y=VsimP1, name="Volume"), row=3, col=1 )
        # Patient 2
        ...
        figPset.add_trace(go.Scatter(x=t,y=VsimP2, name="Volume"), row=3, col=2 )

        # Update xaxis properties
        figPset.update_xaxes(title_text="<b>Time(s)<b>", title_font=dict(size=18), row=3, col=1) # Similar for all graph
        figPset.update_xaxes(title_text="<b>Time(s)<b>", title_font=dict(size=18), row=3, col=2)
        figPMV.update_xaxes(title_text="<b>Time(s)<b>", title_font=dict(size=18), row=3, col=1)

        # Update yaxis properties
        figPset.update_yaxes(title_text="<b>Pressure (cmH2O)<b>", title_font=dict(size=15), row=1, col=1) 
        figPset.update_yaxes(title_text="<b>Flow (L/s)<b>", title_font=dict(size=15),  row=2, col=1) 
        figPset.update_yaxes(title_text="<b>Volume (L)<b>", title_font=dict(size=15),  row=3, col=1) 
        figPMV.update_yaxes(title_text="<b>Pressure (cmH2O)<b>", title_font=dict(size=15), row=1, col=1) 
        figPMV.update_yaxes(title_text="<b>Flow (L/s)<b>", title_font=dict(size=15),  row=2, col=1) 
        figPMV.update_yaxes(title_text="<b>Volume (L)<b>", title_font=dict(size=15),  row=3, col=1) 
